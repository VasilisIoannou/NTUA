...
int main(int argc, char* argv[])
{
	...
	size_t size = sizeof(sharedData) + (y_chars-1)*sizeof(sem_t);
	sharedData *data = create_shared_memory_area(size);


	for(int i=0; i < y_chars; i++){
		sem_init(&data->turn_sem[i],1,i == 0 ? 1 : 0); //Set the first one to 1 only
	}	
	...
	for(int i=0; i < NPROC;i++){
		...
		if(proc[i] == -1){
			...
			// The child process must calculate lines: i, i + NPROC, i+ 2*NPROC, ... , until i + n*NPROC > y_chars 

			// After the calculation is finished wait using semaphores
			// When it is the turn of the this process print line
			// For waiting along with semaphores there needs to be a list of which is the next child to output 
					
			int color_val[y_chars][x_chars];
			for(int line = i;line < y_chars ; line+= NPROC){
			        compute_mandel_line(line, color_val[line]);
				
			}	
			//Here we will have the output
			for(int line = i;line < y_chars ; line+= NPROC){
				sem_wait(&data->turn_sem[line]);
				
				//printf("Line: %d \n",line);
				output_mandel_line(1, color_val[line]);
				if(line + 1 < y_chars){
					sem_post(&data->turn_sem[line+1]);
				}
			}
			exit(0);
	...
	free(proc);
	for(int i=0;i < NPROC ; i++){
		sem_destroy(&data->turn_sem[i]);
	}	
	destroy_shared_memory_area(data, sizeof(sharedData) + (NPROC -1)*sizeof(sem_t));
	reset_xterm_color(1);
	return 0;
}
